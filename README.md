...
[pre-built release](https://github.com/ManuLinares/qori/releases), the heavy
graphics dependencies (such as `wlroots`, `wayland-server`, `libdrm`,
`libdisplay-info`, `libliftoff`, `pixman`, `lcms2`, and `libxkbcommon`) are
statically compiled directly into the binary.

You only need standard hardware-level runtime libraries. On a typical
Ubuntu/Debian desktop environment, these are already pre-installed. If you are
starting from a minimal or server installation, you can install the runtime
dependencies with:
...
Building **qori** locally from source requires the following minimum library
